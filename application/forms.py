...
class UpdateAcademicForm(FlaskForm):
    ...
    academicobject = SelectField('Academic to Update', choices = [], coerce=int) # should return id of academic object
...
class UpdatePaperForm(FlaskForm):
    ...
    paperobject = SelectField('Paper to update', choices = [], coerce = int) 
...
class AddPaper(FlaskForm):
    ...
    impact = IntegerField('Impact Factor(0 <> 20)', validators = [NumberRange(min = 0, max = 20)])
    field_of_study = StringField('Field of Study', validators = [Length(max = 30)])
    authors = SelectField('Authors', choices = [], coerce=int) #shoudl return id of academic object
    submit = SubmitField('')

class DeleteAcademic(FlaskForm):
...
class DeletePaper(FlaskForm):
     ...
     paperobject = SelectField('Paper to update', choices = [], coerce = int) # should retutn id of paper object
     title = StringField('Title of Paper', validators = [Length(max = 30), DataRequired()])
